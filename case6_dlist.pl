...
memberdepd(_,O-_) :- var(O), !, fail.
memberdepd(d([X],A),[d([X],A) | _]-_).
memberdepd(d([X,Y],A),[d([U,V],A) | _]-_) :- u([U,V],X,Y).
memberdepd(d([X,Y,Z],A),[d([U,V,W],A) | _]-_) :- u([U,V,W],X,Y,Z).
memberdepd(d([W,X,Y,Z],A),[d([S,T,U,V],A) | _]-_) :- u([S,T,U,V],W,X,Y,Z).
memberdepd(d([V1,W,X,Y,Z],A),[d([R,S,T,U,V],A) | _]-_) :- u([R,S,T,U,V],V1,W,X,Y,Z).
memberdepd(D,[_ | Ls]-O) :- memberdepd(D,Ls-O).
...
case2(O-O,_,_,100,100,_,_) :- var(O).
case2([d([A],B)|O]-O,A,B,100,1,_,PB) :- var(O), PB=<1.
case2([d([B],A)|O]-O,A,B,1,100,PA,_) :- var(O), PA=<1.
case2([d([A],B), d([B],A)|O]-O,A,B,1,1,PA,PB) :- var(O), PA=<1, PB=<1.

case2f([FL1],A,B,RA,RB) :- case2(FL1-O,A,B,RA,RB,RA,RB), O=[].
...
tvalid(Y,A,B,LYA,LYB) :-
    (memberdepd(d(Y,A),LYA) -> memberdepd(d(Y,B),LYB); true).

case3(LABC-O,LAB,LAC,LBC,A,B,C,RA,RB,RC,PA,PB,PC) :- 
    var(O),
    ...
    (RCm<100 -> (tvalid([A],B,C,LAB,LAC), tvalid([B],A,C,LAB,LBC), LABC2=[d([A,B],C) | O], RC=RCm)
     ; ((PC=<2, tvalid([A],B,C,LAB,LAC), tvalid([B],A,C,LAB,LBC), LABC2=[d([A,B],C) | O], RC=2); (LABC2=O, RC=100))).

case3f([FL2,FL1],A,B,C,RA,RB,RC) :-
    case3(FL2-O,LAB,LAC,LBC,A,B,C,RA,RB,RC,RA,RB,RC),
    O=[],
...
case4(LABCD-O,LABC,LABD,LACD,LBCD,LAB,LAC,LAD,LBC,LBD,LCD,
...
case5(LABCDE-O,LABCD,LABCE,LABDE,LACDE,LBCDE,
...
case6(LABCDEF-O, LABCDE,LABCDF,LABCEF,LABDEF,LACDEF,LBCDEF,
    ...
    (RFm<100 -> (tvalid([A,B,C,D],E,F,LABCDE,LABCDF), 
		 ...
		 LABCDEF5=[d([A,B,C,D,E],F) | O], RF=RFm)
     ;((PF=<5, tvalid([A,B,C,D],E,F,LABCDE,LABCDF), 
	       tvalid([A,B,C,E],D,F,LABCDE,LABCEF), tvalid([A,B,D,E],C,F,LABCDE,LABDEF), 
	       tvalid([A,C,D,E],B,F,LABCDE,LACDEF), tvalid([B,C,D,E],A,F,LABCDE,LBCDEF),
	       LABCDEF5=[d([A,B,C,D,E],F) | O], RF=5); 
       (LABCDEF5=O, RF=100))).

case6f([FL5,FL4,FL3,FL2,FL1],A,B,C,D,E,F,RA,RB,RC,RD,RE,RF) :-
    case6(FL5-O, LABCDE,LABCDF,LABCEF,LABDEF,LACDEF,LBCDEF,
	  LABCD,LABCE,LABCF,LABDE,LABDF,LABEF,LACDE,LACDF,LACEF,LADEF,LBCDE,LBCDF,LBCEF,LBDEF,LCDEF,
	  LABC,LABD,LABE,LABF,LACD,LACE,LACF,LADE,LADF,LAEF,LBCD,LBCE,LBCF,LBDE,LBDF,LBEF,LCDE,LCDF,LCEF,LDEF,
	  LAB,LAC,LAD,LAE,LAF,LBC,LBD,LBE,LBF,LCD,LCE,LCF,LDE,LDF,LEF,
