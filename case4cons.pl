...
case1f([FL0],A,RA) :- case1(FL0,A,RA).

cases1f(Ss,N,A,RA) :- 
    findall(S,case1f(S,A,RA),Ss),length(Ss,N).
...
cases1fr(RSs,RN,A,RA) :-
    cases1f(Ss,_,A,RA), reducecasesf(Ss,RSs), length(RSs,RN).

cases1ff(RSs,RN,A,RA) :- cases1fr(RSs,RN,A,RA).


case2(LAB,LA0,LB0,A,B,RA,RB) :- 

    case1(LA0,A,RAm), case1(LB0,B,RBm),

    (RAm<100 -> (LAB=[d([B],A) | LAB1], RA=RAm)
     ; ((LAB=[d([B],A) | LAB1], RA=1); (LAB=LAB1, RA=100))),
    (RBm<100 -> (LAB1=[d([A],B)], RB=RBm)
     ; ((LAB1=[d([A],B)], RB=1); (LAB1=[], RB=100))),
...
case2f([FL1,FL0],A,B,RA,RB) :- 
    case2(FL1,LA0,LB0,A,B,RA,RB), append(LA0,LB0,FL0).
...
case3(LABC,LAB,LAC,LBC,LA0,LB0,LC0,A,B,C,RA,RB,RC) :- 

    case2(LAB,LA0,LB0,A,B,RA1,RB1), case2(LAC,LA0,LC0,A,C,RA2,RC1), 
    case2(LBC,LB0,LC0,B,C,RB2,RC2),

    min(RA1,RA2,RAm), min(RB1,RB2,RBm), min(RC1,RC2,RCm),
    
    (RAm<100 -> (LABC=[d([B,C],A) | LABC1], RA=RAm)
     ; ((LABC=[d([B,C],A) | LABC1], RA=2); (LABC=LABC1, RA=100))),
    (RBm<100 -> (LABC1=[d([A,C],B) | LABC2], RB=RBm)
     ; ((LABC1=[d([A,C],B) | LABC2], RB=2); (LABC1=LABC2, RB=100))),
    (RCm<100 -> (LABC2=[d([A,B],C)], RC=RCm)
     ; ((LABC2=[d([A,B],C)], RC=2); (LABC2=[], RC=100))),
...
case3f([FL2,FL1,FL0],A,B,C,RA,RB,RC) :-
    case3(FL2,LAB,LAC,LBC,LA0,LB0,LC0,A,B,C,RA,RB,RC),
    append(LAB,LAC,LBC,FL1),
    append(LA0,LB0,LC0,FL0).
...
case4(LABCD,LABC,LABD,LACD,LBCD,LAB,LAC,LAD,LBC,LBD,LCD,LA0,LB0,LC0,LD0,
      A,B,C,D,RA,RB,RC,RD) :- 
    case3(LABC,LAB,LAC,LBC,LA0,LB0,LC0,A,B,C,RA1,RB1,RC1), 
    case3(LABD,LAB,LAD,LBD,LA0,LB0,LD0,A,B,D,RA2,RB2,RD1),
    case3(LACD,LAC,LAD,LCD,LA0,LC0,LD0,A,C,D,RA3,RC2,RD2), 
    case3(LBCD,LBC,LBD,LCD,LB0,LC0,LD0,B,C,D,RB3,RC3,RD3),

    min(RA1,RA2,RA3,RAm), min(RB1,RB2,RB3,RBm),
    min(RC1,RC2,RC3,RCm), min(RD1,RD2,RD3,RDm),

    (RAm<100 -> (LABCD=[d([B,C,D],A) | LABCD1], RA=RAm)
     ; ((LABCD=[d([B,C,D],A) | LABCD1], RA=3); (LABCD=LABCD1, RA=100))),
    (RBm<100 -> (LABCD1=[d([A,C,D],B) | LABCD2], RB=RBm)
     ; ((LABCD1=[d([A,C,D],B) | LABCD2], RB=3); (LABCD1=LABCD2, RB=100))),
    (RCm<100 -> (LABCD2=[d([A,B,D],C) | LABCD3], RC=RCm)
     ; ((LABCD2=[d([A,B,D],C) | LABCD3], RC=3); (LABCD2=LABCD3, RC=100))),
    (RDm<100 -> (LABCD3=[d([A,B,C],D)], RD=RDm)
     ; ((LABCD3=[d([A,B,C],D)], RD=3); (LABCD3=[], RD=100))),
...
case4f([FL3,FL2,FL1,FL0],A,B,C,D,RA,RB,RC,RD) :-
    case4(FL3,LABC,LABD,LACD,LBCD,LAB,LAC,LAD,LBC,LBD,LCD,LA0,LB0,LC0,LD0,
	  A,B,C,D,RA,RB,RC,RD),
    append(LABC,LABD,LACD,LBCD,FL2), append(LAB,LAC,LAD,LBC,LBD,LCD,FL1),
    append(LA0,LB0,LC0,LD0,FL0).
...
case4fr(RFL3,RFL2,RFL1,RFL0,A,B,C,D,RA,RB,RC,RD) :- 
    cases4fr(RSs,_,A,B,C,D,RA,RB,RC,RD), 
    member([RFL3,RFL2,RFL1,RFL0],RSs).
